### 26/08/19
* ランダムで「スリングショット」「マイクロビキニ」を水着の代わりに着用するよう変更
> 露出癖Lv3以上、またはエロ制服許可フィールド展開中かつ、該当するグラフィックがあることが条件

### 26/07/19
* 秘書業務キャラの朝の目覚まし機能の拡充
> 目覚まし行為に「パイズリ」「手コキ」を追加
> 「フェラチオ」の行為文に分岐を追加
> 自主的に行った行為を記録し、従業員の仕事設定画面から記録された行為を依頼できるように。
> 秘書朝起こし画像が表示されないバグを修整し、全ての行為に対し「秘書朝起こし」→「コマンド実行時」→「短冊」と画像をサーチするように関数化。
* 画像ファイル名チェック機能に、画像の実装状況をカウントして表にする機能を追加
> 衣装/差分の組み合わせごとにカウントします。とても重いので注意。
> 短冊画像、秘書朝起こし画像、コマンド実行時画像に対応。コマンド実行時_プレイヤー側画像には非対応（まとめてカウント）
> 負荷軽減のため、差分なし短冊画像（「短冊_水着.webp」など）のない衣装は全スキップします。コマンド実行時画像は対応する画像が既にあるコマンドのみカウントします。
* 関数「HTMLボタン_条件付き」を追加
> HTMLボタン_条件付き(ONOFF条件, ボタン値, 表示文, [解説文, 単ボタン化, ボタン化条件, 隠蔽フラグ])
> []内は省略可。ボタン値、表示文、解説文は文字列型。
* 「___キャラ画像の入れ方.txt」「__キャラ画像をAIで作成するための手引.txt」の加筆修整
> 秘書の目覚まし行為用画像の入れ方と新規衣装のプロンプトがメイン

### 26/06/19
* 能力表示画面.webpの表示領域を1：2に調整
* 画像ファイル名チェックで全裸（衣装限定）のファイル名で、実装されていない衣装名で命名されていた場合を検出できるように。

### 26/04/19
* 「___キャラ画像の入れ方.txt」の改修
> 画像関係のシステムに色々と手が入っていたので、現在の仕様に即するように記述変更。
* 衣装「オルターエゴ・マリシャス」の追加

### 25/12/19
* 差分エフェクトのON/OFF切替を個別に設定できるように
> 「OPTION→機能切り替え→画像関連→オーバーレイ設定」から設定できる他、立ち絵に表示されている状態では画像下の切り替えボタンから変更できるように。  
> 他の方の機能追加で立ち消えたのでToDoリスト行き

### 25/11/19
* AIイラスト作成の手引を作成
> 一からの説明ではなく、どの衣装差分を出すのにどんなプロンプトを使うのかって部分が主。
> 自分用の備忘録も兼ねて作成。
* エロ衣装「蠱惑の舞姫」追加
> Discord鯖でベリーダンサーコスプレ画像いっぱい流れてくるんで、ムラムラしてやった。反省はしてない。
* 全裸画像を表示するとき、直前まで着ていた衣装によって表示する画像を変更できる機能を追加
> 「顔_全裸（水着）.webp」などのように、「全裸」の後に全角カッコで脱ぐ前の衣装名を記述すること
* 横幅が180px以外の顔グラフィック表示時に差分エフェクトがズレて表示される不具合の修正
* 外部作品フィルタ機能で、外部作品の一覧を文字コード順に並べ直してから表示するよう変更
* 関数「ページ送り共通表示」「ページ送り共通実行」を追加
> ページ送り表示部分と実行部分を外部関数可して共通フォーマットで実行可能に 
> 遊んでいてページ送り機能が使いづらいと感じた機能を順繰りに置き換えていきたい
* 一言コメントに「絶頂時」「ダウン中」「睡眠中（恋人・セフレ時）」を追加
* 汎用喘ぎにCSTR:一人称、二人称を使用するよう変更
