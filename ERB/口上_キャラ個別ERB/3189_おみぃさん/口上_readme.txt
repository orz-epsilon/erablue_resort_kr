...
;パッチ名：子供に囲まれて幸せに過ごせおみぃさん
;作者：Alno30
;作成日：2026/1/23～2026/2/23
;
;作成内容の概要：
;おみぃさんを実装したいので作り申した
;とりあえず最低限、日常コマンドだの性行為系だのを記述した形です。
;そこのあなたもこの口上に追記しよう！自認おみぃさんになって書くだけだから！簡単だから！
;_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/#
;ライセンスについて
...
;	+-------------------+-------------------------------------------------------------------+
;	|IMAGEPATH_         |直後の文字列で指定した画像を原寸大で表示します
;	+-------------------+-------------------------------------------------------------------+
;	|VIDEOPTH_          |直後の文字列で指定したフォルダの動画を再生します
;	+-------------------+-------------------------------------------------------------------+
;
; この表形式めんどくせぇからここから下普通に書くね
;「BIG_」
;　後ろの文字列を大文字で表示する。行内部で普通サイズの文字と併用することは想定されていない。
;
;「_BIGUP_」
;　後ろの文字列を大文字で表示する。既存行より上にはみ出して表示される。上に空行を入れておかないと文字がかぶる危険性あり。
;
;「_BIGDOWN_」
;　後ろの文字列を大文字で表示する。既存行より下にはみ出して表示される。下に空行を入れておかないと文字がかぶる危険性あり。
;
;
